...
function openBlock() {
  document.getElementById("animated-block").style.display = "block";
}

// Функция закрытия блока
function closeBlock() {
  var animatedBlock = document.getElementById("animated-block");
  animatedBlock.style.animation = "slide-out 0.5s ease-out forwards";
  setTimeout(function() {
  animatedBlock.style.display = "none";
  animatedBlock.style.animation = "";
  }, 500);
  }

// Привязываем открытие блока к событию клика на текст
document.getElementById("clickable-text").addEventListener("click", openBlock);
